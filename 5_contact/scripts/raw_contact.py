...
import tqdm
import os
from scripts.utils import count_fasta_sequences, open_input
import logging
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 9:
        print("Usage: raw_contact.py <bam> <enzymes> <fasta> <output_dir> <min_mapq> <min_len> <min_match> <min_signal>")
        sys.exit(1)

    bam_file = sys.argv[1]
    enzymes = sys.argv[2].split(',')  # assume comma-separated
    fasta = sys.argv[3]
    out_dir = sys.argv[4]
    min_mapq = int(sys.argv[5])
    min_len = int(sys.argv[6])
    min_match = int(sys.argv[7])
    min_signal = int(sys.argv[8])

    print("[INFO] Running ContactMatrix...")
    cm = ContactMatrix(
        bam_file=bam_file,
        enzymes=enzymes,
        seq_file=fasta,
    ...
    print("[INFO] ContactMatrix finished.")
