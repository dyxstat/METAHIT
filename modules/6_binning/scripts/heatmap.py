...
def plot(sparse_mat, contig_to_bin, order, idx_to_name, outdir, max_image_size=5000, width=25, height=22, dpi=300):
    ...
    seaborn.heatmap(mat, square=True, cmap="rocket", ax=ax, cbar=False, linewidths=0)
    ...
    if len(tick_locs) == len(bin_ids) + 1:
        mids = [tick_locs[i] + (tick_locs[i+1] - tick_locs[i]) // 2 for i in range(len(bin_ids))]
        ax.yaxis.set_minor_locator(ticker.FixedLocator(mids))
        ax.yaxis.set_minor_formatter(ticker.FixedFormatter([f"CL{i+1:04}" for i in range(len(bin_ids))]))
