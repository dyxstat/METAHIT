...
from ImputeCC.imputation import ImputeMatrix
from ImputeCC.exceptions import ApplicationException
from ImputeCC.pre_common import get_bin_dirs
from ImputeCC.pre_profile import Profile
from ImputeCC.utility import save_object, load_object, make_dir, gen_bins
from ImputeCC.final_cluster import FinalCluster
from ImputeCC.pre_clustering import Clust4CheckM, PreCluster
...
if __name__ == '__main__':
    ...
    # <<< added for CheckM DB
    global_parser.add_argument('--checkm_db', type=str, default=None,
                               help='Path to CheckM database directory (optional)')
    ...
    global_parser.add_argument('--imputecc-gene-coverage', type=float, default=0.9)
    global_parser.add_argument('--imputecc-rwr-restart-probability', type=float, default=0.5)
    global_parser.add_argument('--imputecc-rwr-threshold', type=float, default=80)
    global_parser.add_argument('--imputecc-max-markers', type=int, default=8000)
    global_parser.add_argument('--imputecc-intra-bin-threshold', type=float, default=50)
    global_parser.add_argument('--imputecc-inter-bin-threshold', type=float, default=0)
    global_parser.add_argument('--imputecc-min-bin-size', type=int, default=100000)
    global_parser.add_argument('--imputecc-contamination-weight', type=float, default=2)
    global_parser.add_argument('--imputecc-min-completeness', type=float, default=50)
    global_parser.add_argument('--imputecc-max-contamination', type=float, default=10)
    global_parser.add_argument('--imputecc-report-quality-threshold', type=float, default=10)
    ...
    global_parser.add_argument('-t', '--threads', default=16, type=int, help="the number of threads. default is 16.")
    ...
    for folder in [metacc_folder, bin3c_folder, imputecc_folder]:
        try:
            make_dir(folder)
        except IOError:
            print(f'Error: cannot find or create {folder}')
            sys.exit(1)
    
    for tmp_dir in [metacc_temp_folder, os.path.join(imputecc_folder, 'tmp')]:
    ...
    bin3c_runtime_defaults = {'min_reflen': 1000,'min_signal': 5,'min_extent': 50000,'min_mapq': 60,'strong': 10}
    metacc_runtime_defaults = {'min_len': 1000,'min_signal': 2,'min_mapq': 30,'min_match': 30,'thres': 0.05,'min_binsize':150000}
    ...
    try:
        ...
        hzmap = NormCCMap(metacc_folder, contig_info_df, cm.seq_map_metacc, norm_result, thres = ifelse(args.thres, metacc_runtime_defaults['thres']))
        ...
        if not args.no_fasta:
            write_fasta(cm, bin3c_folder, clustering, source_fasta=args.FASTA, clobber=True, only_large=args.only_large)
        ...
        imp = ImputeMatrix(CONTIG_INFO, HIC_MATRIX, marker_file,
                            gene_cov=args.imputecc_gene_coverage,
                            rwr_rp=args.imputecc_rwr_restart_probability,
                            rwr_thres=args.imputecc_rwr_threshold,
                            max_markers=args.imputecc_max_markers)
        ...
        bins, bin_of_contigs = PreCluster(imp.marker_contig_counts, imp.marker_contigs, imp.contig_markers,
                                         ...
                                         intra=args.imputecc_intra_bin_threshold,
                                         inter=args.imputecc_inter_bin_threshold)
        ...
        if args.checkm_db:
            ...
            logger.info(f"Using CheckM database: {args.checkm_db}")
        else:
            logger.info("Using default CheckM database path from environment.")

        if not (os.path.exists(checkm_ar_gene_table) and os.path.exists(checkm_bac_gene_table)):
            Clust4CheckM(args.FASTA, CONTIG_INFO, HIC_MATRIX, imputecc_folder)
            output_dir = os.path.join(os.path.join(imputecc_folder , 'tmp'), 'out_checkm_qa')
            bin_file = os.path.join(os.path.join(imputecc_folder , 'tmp') , 'dir4checkm.tsv')
            cpus = args.threads
            ...
            shutil.move(
                os.path.join(output_dir, 'binning_methods', 'INITIAL_BIN', 'checkm_bac', 'marker_gene_table.tsv'),
                checkm_bac_gene_table,
            )
            shutil.move(
                os.path.join(output_dir, 'binning_methods', 'INITIAL_BIN', 'checkm_ar', 'marker_gene_table.tsv'),
        ...
        cluster_process = FinalCluster(imp.contig_info, imp.contig_local, 
             ...
             intra=args.imputecc_intra_bin_threshold,
             inter=args.imputecc_inter_bin_threshold,
             cont_weight=args.imputecc_contamination_weight,
             min_comp=args.imputecc_min_completeness,
             max_cont=args.imputecc_max_contamination,
             report_quality=args.imputecc_report_quality_threshold,
             min_binsize=args.imputecc_min_bin_size)
        ...
        for file in os.listdir(os.path.join(bin3c_folder,"fasta")):
            if file.endswith(".fna"):
                 replace_in_file(os.path.join(os.path.join(bin3c_folder,"fasta"), file))
