...
def parse_args():
    ...
    parser.add_argument("-m", "--memory", type=int, default=DEFAULT_MEMORY_GB)
...
def write_seqtk_subsequence(reads, readnames, destination):
    """Compress seqtk's stdout through Python instead of bypassing gzip."""
    argv = ["seqtk", "subseq", str(reads), str(readnames)]
    print(f"[CMD] {shlex.join(argv)} > {destination}", flush=True)
...
def ensure_name_sorted_bam(bam_path, outdir, threads, already_name_sorted=False):
    ...
    run_command(["samtools", "sort", "-n", "-@", threads, "-o", name_sorted_bam, bam_path])
...
def write_selected_readnames_from_insert_table(insert_table, cutoff, out_readnames):
    ...
    with open_text(insert_table, "rt") as f:
        header = f.readline().rstrip("\n").split("\t")
        ...
        for line in f:
            ...
            if d <= cutoff:
                selected_names.add(fields[rid_idx])
                selected += 1
    with open(out_readnames, "w") as output:
        for read_name in sorted(selected_names):
            output.write(read_name + "\n")

    selected_unique = len(selected_names)
...
def main():
    ...
    summary = {
        ...
        "insert_size_counts": counts,
        "em_model": em_params,
        "selection": {
            "top_insert_sizes_used_for_em": int(len(top_insert_sizes)),
            "all_intra_insert_pairs": int(all_intra_written),
            "selected_pairs": int(selected_pairs),
            "selected_fraction_of_intra_pairs": finite_or_none(selected_fraction_intra),
            "selected_fraction_of_filtered_pairs": finite_or_none(selected_fraction_filtered),
            "informative_fraction": finite_or_none(informative_fraction),
            "non_informative_fraction": (
                float(1 - informative_fraction)
                if np.isfinite(informative_fraction)
                else None
            ),
            "three_dimensional_contact_ratio": finite_or_none(ratio_3d),
            "insert_size_cutoff": float(cutoff),
        },
    }

    with open(os.path.join(args.outdir, "read_selection_summary.json"), "w") as f:
    ...
    write_seqtk_subsequence(args.hic1, readnames, sg_in_hic_fwd)
    write_seqtk_subsequence(args.hic2, readnames, sg_in_hic_rev)

    for selected_path, shotgun_path, combined_path in (
        (sg_in_hic_fwd, args.sg1, combined_sg_fwd),
        (sg_in_hic_rev, args.sg2, combined_sg_rev),
    ):
        with open(combined_path, "wb") as output:
            for input_path in (selected_path, shotgun_path):
    ...
    if args.write_nonselected_hic:
        ...
        all_names = set()
        with pysam.AlignmentFile(name_sorted_bam, "rb") as bam_handle:
            for alignment in bam_handle.fetch(until_eof=True):
                if not alignment.is_secondary and not alignment.is_supplementary:
                    all_names.add(alignment.query_name)
        selected_names = {line.strip() for line in open(readnames) if line.strip()}
        with open(all_readnames, "w") as handle:
            handle.writelines(f"{name}\n" for name in sorted(all_names))
        with open(non_selected_readnames, "w") as handle:
            handle.writelines(f"{name}\n" for name in sorted(all_names - selected_names))
        ...
        write_seqtk_subsequence(args.hic1, non_selected_readnames, non_sg_fwd)
        write_seqtk_subsequence(args.hic2, non_selected_readnames, non_sg_rev)
    ...
    reassemble_cmd = [
        sys.executable,
        os.path.join(args.metahict_path, "7_reassembly/reassemble_bins.py"),
        "-b", args.bin,
        "-o", args.outdir,
        "-1", combined_sg_fwd,
        "-2", combined_sg_rev,
        ...
        "-c", args.min_completeness,
        "-x", args.max_contamination,
        "--spades-mode", args.spades_mode,
        "--parallel",
    ]

    if args.checkm2_db:
        reassemble_cmd.extend(["--checkm2_db", args.checkm2_db])
    if args.skip_checkm2:
        reassemble_cmd.append("--skip-checkm")
    if args.tmp_dir:
        reassemble_cmd.extend(["--tmp-dir", args.tmp_dir])
    if args.spades_phred_offset:
        reassemble_cmd.extend(["--spades-phred-offset", args.spades_phred_offset])
    if args.spades_extra_args:
        reassemble_cmd.extend(["--spades-extra-args", args.spades_extra_args])
    if args.skip_residual_assembly:
        reassemble_cmd.append("--skip-residual-assembly")
    if args.keep_temp:
        reassemble_cmd.append("--keep-temp")
